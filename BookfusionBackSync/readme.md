...
Данные логина/пароля используются **только** для авторизации в BookFusion API и
не отправляются разработчику плагина.
...
| Completed Threshold (%) | Порог процента дочитывания для Completed (по умолчанию `98.9`) |
| Fetch Page Size | Размер страницы при загрузке `/v3/library/books.json` (по умолчанию `1337`) |
| Fetch Timeout (sec) | Таймаут запроса загрузки библиотеки в секундах (по умолчанию `90`) |
| Full SKIP Logs | Логировать все `SKIP`-строки (иначе пишется ограниченная выборка) |
| Batch UI Logs | Группировать обновления лога в UI; размер батча динамический и не более `0.5%` от общего числа книг |
...
Иными словами: пароль хранится локально на вашем компьютере и используется только
для запросов к BookFusion.
...
В диалоге два прогресс-бара:
- верхний — загрузка библиотеки из BookFusion,
- нижний — сопоставление и запись в Calibre.

Оба прогресс-бара обновляются только при смене целого процента.
...
Если `Full SKIP Logs` выключен, в файл-лог пишется только ограниченная выборка `SKIP`-строк,
а полная статистика остается в итоговой строке `Match stats`.
